...
class Strategy:
    ...
    def getPlay(
        ...
        if DEBUG:
            print("Dealer shown card: ", dealerCard.getFullName())
    ...
    def getDealerPlay(
        self, hand: list = [], knownCards: list = [], dealerCard: Card = None
    ) -> int:
        if self.score < 17:
            return gc.HIT
        else:
    ...
    def getPlayFromRules(self, ruleNo: int) -> int:
        if self.rules is None or len(self.rules) == 0 or ruleNo >= len(self.rules):
            ...
            action = self.rules[ruleNo].evaluate(
                self.score, self.dealer, self.knowncount, self.soft, self.hand
            )
