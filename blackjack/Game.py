...
def dealOpeningHands(players: list = [], dealer: Player = None, deck: Deck = None):
    ...
    if deck.noOfDecks == 1:
        deck.shuffle()
    elif (deck.discarded / len(deck.cards)) >= gc.SHUFFLE_POINT:
        deck.shuffle()
...
def getResults(players: list, dealer: Player):
    ...
    for player in players:
        ...
        while handNo < len(player.hands):
            ...
            if DEBUG:
                print(player.name, "hand #: ", handNo, " score: ", score)
            if score > 0:
...
def cleanupGame(
    players: list = None, dealer: Player = None, deck: Deck = None, betSizes: list = []
):
...
def main(confFile: cp.ConfigParser = None):
    print("running sim")
    suites, valDict = util.getDeckFromXML(
        confFile.get(gc.GameConst, gc.deck, fallback=DEFAULT_DeckFile).strip()
    )
    deck = Deck(
        suites, valDict, int(confFile.get(gc.GameConst, gc.noOfDecks, fallback=1))
    )
    # setup players
    strats = []
    betSizes = []
    ...
    game = 0
    while game < int(
        confFile.get(gc.GameConst, gc.handsToPlay, fallback=DEFAULT_HandsPlayed)
    ):
        ...
        RESULT_DICT["table_history"].append(
            (RESULT_DICT["total_winnings"], dealer.getScore())
        )
        RESULT_DICT["player_histories"] = [player.winningsHistory for player in players]
...
if __name__ == "__main__":
    ...
    for file in filesToRun:
        ...
        filewithext = file
        if not filewithext.endswith(".txt"):
            filewithext = filewithext + ".txt"
        ...
        if config is not None:
            ...
            gc.BJ_MOD = float(
                config.get(gc.GameConst, gc.blackjackModTag, fallback=1.5)
            )
            gc.SHUFFLE_POINT = float(
                config.get(gc.GameConst, gc.shuffleTag, fallback=0.0)
            )
            # Run sim
            ...
            if PLOT:
                ...
                x = np.linspace(
                    1,
                    len(RESULT_DICT["player_histories"][0]),
                    len(RESULT_DICT["player_histories"][0]),
                )
                ...
                for hist in RESULT_DICT["player_histories"]:
                    ax.plot(
                        x,
                        [entry[0] for entry in hist],
                        label=RESULT_DICT["player_names"][i],
                        marker=".",
                        linewidth=2,
                        markersize=2,
                    )
                ...
                ax2.plot(
                    x,
                    [-1 * entry[0] for entry in RESULT_DICT["table_history"]],
                    c=housecolor,
                    label="House",
                    marker=".",
                    linewidth=2,
                ...
                maxVal = max(
                    map(abs, [entry[0] for entry in RESULT_DICT["table_history"]])
                )
            ...
            if SAVE:
                writeResultDict(
                    RESULT_DICT, os.path.join(RESULT_DIR, filename + ".txt"), players
                )
        else:
